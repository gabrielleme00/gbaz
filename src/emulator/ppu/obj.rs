...
const PRAM_OFFSET_FG: u32 = 0x200;
...
impl ObjAttrs {
    ...
    fn is_obj_window(&self) -> bool {
        ObjMode::from(self.0.objmode()) == ObjMode::Window
    }
}
...
impl Ppu {
    fn get_affine_matrix(&mut self, affine_index: u32) -> AffineMatrix {
        ...
        let pa = self.oam_read_16(offset) as i16 as i32;
        offset += 2 + AFFINE_FILL;
        let pb = self.oam_read_16(offset) as i16 as i32;
        offset += 2 + AFFINE_FILL;
        let pc = self.oam_read_16(offset) as i16 as i32;
        offset += 2 + AFFINE_FILL;
        let pd = self.oam_read_16(offset) as i16 as i32;
    ...
    fn read_obj_attrs(&mut self, obj: usize) -> ObjAttrs {
        ...
        let attr0 = Attribute0(self.oam_read_16(addr));
        let attr1 = Attribute1(self.oam_read_16(addr + 2));
        let attr2 = Attribute2(self.oam_read_16(addr + 4));
    ...
    fn render_affine_obj(&mut self, attrs: ObjAttrs, _obj_num: usize) {
        ...
        let (bbox_w, bbox_h) = match ObjType::from(attrs.0.objtype()) {
        ...
        if ObjMode::from(attrs.0.objmode()) == ObjMode::Forbidden {
            return;
        }
        ...
        let tile_array_width = match self.dispcnt.obj_character_mapping() {
            // 1D
            true => obj_w / 8,
            // 2D
            false => {
            ...
            ($read_pixel_index_fn:ident) => {
                for ix in (-half_width)..(half_width) {
                    ...
                    if texture_x >= 0 && texture_x < obj_w && texture_y >= 0 && texture_y < obj_h {
                        ...
                        let pixel_color =
                            self.get_palette_color(pixel_index as u32, palette_bank, PRAM_OFFSET_FG);
    ...
    pub(in super::super) fn render_objs(&mut self) {
        for obj_num in 0..128 {
            ...
            match ObjType::from(obj.0.objtype()) {
